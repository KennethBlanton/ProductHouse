// src/components/ui/toast.tsx
import React, { useEffect, useState } from 'react';

type ToastVariant = 'info' | 'success' | 'warning' | 'error';

interface ToastProps {
  id: string;
  variant?: ToastVariant;
  title?: string;
  message: string;
  duration?: number;
  onClose: (id: string) => void;
}

const variantClasses: Record<ToastVariant, { icon: JSX.Element; bg: string; title: string }> = {
  info: {
    icon: (
      <svg className="w-5 h-5 text-blue-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
      </svg>
    ),
    bg: 'bg-blue-50 dark:bg-blue-900/50',
    title: 'text-blue-900 dark:text-blue-100',
  },
  success: {
    icon: (
      <svg className="w-5 h-5 text-success-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
      </svg>
    ),
    bg: 'bg-success-50 dark:bg-success-900/50',
    title: 'text-success-900 dark:text-success-100',
  },
  warning: {
    icon: (
      <svg className="w-5 h-5 text-warning-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
      </svg>
    ),
    bg: 'bg-warning-50 dark:bg-warning-900/50',
    title: 'text-warning-900 dark:text-warning-100',
  },
  error: {
    icon: (
      <svg className="w-5 h-5 text-error-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
      </svg>
    ),
    bg: 'bg-error-50 dark:bg-error-900/50',
    title: 'text-error-900 dark:text-error-100',
  },
};

export default function Toast({
  id,
  variant = 'info',
  title,
  message,
  duration = 5000, // 5 seconds default
  onClose,
}: ToastProps) {
  const [isVisible, set